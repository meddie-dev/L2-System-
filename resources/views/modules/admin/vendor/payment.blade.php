<x-layout.mainTemplate>
  ...
  <div class="card-body tw-px-4">
    <div class="tw-overflow-x-auto tw-mb-6">
      ...
      <div class="tw-bg-gray-500 tw-rounded-lg tw-px-4 tw-py-3 tw-my-6 tw-text-white | max-md:tw-p-4">
        <h2 class="tw-text-md tw-font-semibold tw-mb-1 | max-md:tw-text-sm">Payment</h2>
        <p class="tw-text-xs | max-md:tw-text-xs">View and manage your payment details.</p>
      </div>
      ...
      <div class="tw-grid tw-grid-cols-1 tw-gap-4 tw-px-4 tw-text-sm | max-md:tw-text-xs max-md:tw-gap-2">
        ...
        <div>
          ...
          <input type="text" id="deliveryLocation" name="deliveryLocation" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-opacity-50 tw-cursor-not-allowed | max-md:tw-text-xs" value="{{ optional(App\Models\User::find($payment->reviewed_by))->roles->pluck('name')->first() ?? 'N/A' }}" readonly>
        </div>
      </div>
      @endif
      ...
      <div class="tw-grid tw-grid-cols-1 tw-gap-4 tw-px-4 tw-text-sm | max-md:tw-text-xs max-md:tw-gap-2">
        ...
        <div>
          <label for="deliveryLocation" class="tw-block tw-text-sm tw-font-medium tw-text-gray-700 | max-md:tw-text-xs">Role</label>
          <input type="text" id="deliveryLocation" name="deliveryLocation" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-opacity-50 tw-cursor-not-allowed | max-md:tw-text-xs" value="{{ optional(App\Models\User::find($payment->approved_by))->roles->pluck('name')->first() ?? 'N/A' }}" readonly>
        </div>
      </div>
      @endif
      ...
      <div class="tw-flex tw-justify-end tw-mb-6">
        <form class="tw-mr-2" action="{{ route('admin.vendors.payment.rejected',[$user->id,$payment->id]) }}" method="POST">
          @csrf
          @method('PATCH')
        ...
        <form action="{{ route('admin.vendors.payment.approved',[$user->id,$payment->id]) }}" method="POST">
          @csrf
          @method('PATCH')
