  ...
  <div>
      ...
      <div class="tw-mt-6">
        <div class="tw-grid tw-grid-cols-2 tw-gap-3 tw-text-sm | max-md:tw-grid-cols-1 max-md:tw-gap-2 ">
          ...
          <div class="tw-mb-4">
            ...
            <input type="text" id="firstName" name="firstName" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-focus:ring-indigo-500 tw-focus:border-indigo-500 | max-md:tw-text-xs" placeholder="Enter first name" value="{{ $user->firstName }}" readonly>
          </div>
          ...
          <div class="tw-mb-4">
            ...
            <input type="text" id="lastName" name="lastName" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-focus:ring-indigo-500 tw-focus:border-indigo-500 | max-md:tw-text-xs" placeholder="Enter last name" value="{{ $user->lastName }}" readonly>
          </div>
          ...
          <div class="tw-mb-4">
            ...
            <input type="email" id="email" name="email" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-focus:ring-indigo-500 tw-focus:border-indigo-500 | max-md:tw-text-xs" placeholder="Enter email address" value="{{ $user->email }}" readonly>
          </div>
          ...
          <div class="tw-mb-4">
            ...
            <select id="driverType" name="driverType" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-focus:ring-indigo-500 tw-focus:border-indigo-500 | max-md:tw-text-xs" required disabled>
          ...
          <tbody id="orderRecords" class="tw-bg-white">
            ...
            <tr class="tw-bg-gray-100">
              ...
              <td class="tw-px-4 tw-py-2 tw-font-bold">
                <span style="color: {{ $ticket->status === 'pending' ? '#F57C00' : ($ticket->status === 'scheduled' ? '#388E3C' : ($ticket->status === 'delivered' ? '#4CAF50' : '#E53935')) }}">{{ ucfirst($ticket->status) }}</span>
              </td>
            </tr>
            @endforeach
          </tbody>
    ...
    <div class="tw-flex tw-justify-start">
      <div class="tw-flex tw-items-center tw-justify-start tw-my-6">
        <a href="{{ route('superadmin.fleet.driver.index') }}" class="tw-flex tw-items-center tw-space-x-1 tw-text-sm tw-font-medium tw-text-gray-200  tw-bg-gray-600 tw-rounded-md tw-px-4 tw-py-2 hover:tw-border hover:tw-border-gray-600 hover:tw-bg-white  hover:tw-text-gray-600 | max-md:tw-p-3 ">
