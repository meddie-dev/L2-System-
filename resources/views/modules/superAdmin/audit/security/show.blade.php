    ...
    <ol class="tw-inline-flex tw-items-center tw-space-x-1 | md:tw-space-x-2 rtl:tw-space-x-reverse max-sm:tw-text-sm">
      ...
      <x-partials.breadcrumb :active="true" :isLast="true">
        <div class="tw-flex tw-items-center tw-justify-center ">
          User (<span class="tw-font-semibold tw-text-xs tw-opacity-20 tw-mt-1 ">{{ $user-> firstName }} {{ $user-> lastName }}</span>)
        </div>
      </x-partials.breadcrumb>
    </ol>
  </nav>
  ...
  <div class="card-body tw-px-4">
          ...
          <div class="tw-container tw-mx-auto tw-p-6">
            <div class="tw-relative tw-border-l-4 tw-border-gray-500 tw-ml-1">
              ...
              <div class="tw-mb-6 tw-ml-6">
                @if (strpos($log->event, 'Unauthorized access attempt') !== false || strpos($log->event, 'Tried to view Unowned File') !== false)
                ...
                @endif
                <div class="tw-bg-white tw-shadow-md tw-rounded-lg tw-p-4" style="background-color: {{ strpos($log->event, 'Unauthorized access attempt') !== false || strpos($log->event, 'Tried to view Unowned File') !== false ? '#f8d7da' : '#ffffff' }};">
    ...
    <div>
      <div class="tw-flex tw-items-center tw-justify-between tw-my-6">
        <a href="{{ route('superadmin.audit.index') }}" class="tw-flex tw-items-center tw-space-x-1 tw-text-sm tw-font-medium tw-text-gray-200  tw-bg-gray-600 tw-rounded-md tw-px-4 tw-py-2 hover:tw-border hover:tw-border-gray-600 hover:tw-bg-white  hover:tw-text-gray-600 | max-md:tw-p-3 ">
          <i class="fa-solid fa-arrow-left tw-mr-2 | max-md:tw-text-xs"></i>
          Back
        </a>
        <form action="{{ route('superadmin.audit.penalty', $user->id) }}" method="POST" class="tw-flex tw-items-center tw-space-x-1 tw-text-sm tw-font-medium tw-text-gray-200 tw-bg-red-600 tw-rounded-md tw-px-4 tw-py-2 hover:tw-border hover:tw-border-gray-600 hover:tw-bg-white hover:tw-text-gray-600 | max-md:tw-p-3">
          @csrf
          @method('PATCH')
          <button type="submit" class="tw-flex tw-items-center">
            <i class="fa-solid fa-ban tw-mr-2 | max-md:tw-text-xs"></i>
            Penalty
          </button>
        </form>
      </div>
