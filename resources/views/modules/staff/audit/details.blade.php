<x-layout.mainTemplate>
  <nav class="tw-flex tw-justify-between |  max-md:tw-hidden" aria-label="Breadcrumb">
    <ol class="tw-inline-flex tw-items-center tw-space-x-1 | md:tw-space-x-2 rtl:tw-space-x-reverse max-sm:tw-text-sm">
      ...
      <x-partials.breadcrumb href="{{ route('staff.audit.index') }}" :active="false" :isLast="false">
        Report Management
      </x-partials.breadcrumb>
    ...
    <div class="tw-overflow-x-auto tw-mb-6">
      ...
      <div class="tw-grid tw-grid-cols-2 tw-gap-4 tw-px-4 tw-text-sm tw-my-4 | max-md:tw-text-xs ">
        ...
        <div>
          ...
          <input type="text" id="incident_type" name="incident_type" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-opacity-50 tw-cursor-not-allowed | max-md:tw-text-xs" placeholder="ENTER INCIDENT TYPE" value="{{ $incidentReport->incident_type ?? 'N/A' }}" readonly>
        </div>
      </div>
      ...
      <div class="tw-flex tw-justify-end tw-mb-6">
        <form class="tw-mr-2" action="{{ route('staff.audit.report.rejected', $incidentReport->id) }}" method="POST">
          @csrf
          @method('PATCH')
        ...
        <form action="{{ route('staff.audit.report.reviewed', $incidentReport->id) }}" method="POST">
          @csrf
          @method('PATCH')
      ...
      <div>
        <div class="tw-flex tw-items-center tw-justify-start tw-my-6">
          <a href="{{ route('staff.audit.index') }}" class="tw-flex tw-items-center tw-space-x-1 tw-text-sm tw-font-medium tw-text-gray-200  tw-bg-gray-600 tw-rounded-md tw-px-4 tw-py-2 hover:tw-border hover:tw-border-gray-600 hover:tw-bg-white  hover:tw-text-gray-600 | max-md:tw-p-3 ">
