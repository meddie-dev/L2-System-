          ...
          <tr>
            ...
            <td class="tw-px-4 tw-py-2">
              <a href="{{ route('staff.vehicleReservation.createOrder', $order->id) }}">
                {{ $order->orderNumber }} <span class="tw-text-gray-400">(
                  @if($order->payment->approval_status === 'approved' && $order->document->first()->approval_status === 'approved')
            ...
            <td class="tw-px-4 tw-py-2">{{ $order->updated_at->format('M d, Y') }}</td>
          </tr>
          @endif
          @endforeach
        </tbody>
