<x-layout.portal.mainTemplate>
    ...
    <ol class="tw-inline-flex tw-items-center tw-space-x-1 | md:tw-space-x-2 rtl:tw-space-x-reverse max-sm:tw-text-sm">
      ...
      <x-partials.breadcrumb :active="true" :isLast="true">
        <div class="tw-flex tw-items-center tw-justify-center ">
          Edit Payment (<span class="tw-font-semibold tw-text-xs tw-opacity-20 tw-mt-1 ">{{ $vehicleReservation->payment->paymentNumber }}</span>)
        </div>
      </x-partials.breadcrumb>
    </ol>
  </nav>
  ...
  <div class="tw-px-4">
    ...
    <form action="{{ route('vendorPortal.vehicleReservation.payment.update', $vehicleReservation->id) }}" enctype="multipart/form-data" method="POST" class="tw-mt-6">
      @csrf
      @method('PATCH')
      <div class="tw-text-sm">
        ...
        <div class="tw-mb-4">
          ...
          <input type="date" id="date" name="date" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm | max-md:tw-text-xs @error('date') is-invalid @enderror" value="{{ old('date', $vehicleReservation->payment->date) }}" required>
          @error('date')
          <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
          @enderror
        </div>
        ...
        <div class="tw-mb-4">
          ...
          <input type="text" id="account_number" name="account_number" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm | max-md:tw-text-xs @error('account_number') is-invalid @enderror" placeholder="Enter Account Number" min="17" max="17" value="{{  $vehicleReservation->payment->account_number }}" required>
        ...
        <div class="tw-mb-2">
          ...
          <input type="number" id="total_amount_due" name="total_amount_due" class="tw-block tw-w-full tw-px-4 tw-py-2 tw-border tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-opacity-50 tw-cursor-not-allowed  | max-md:tw-text-xs @error('total_amount_due') is-invalid @enderror" value="{{ old('total_amount_due', $vehicleReservation->payment->total_amount_due) }}" readonly>
