<x-layout.portal.mainTemplate>
  ...
  <div class="card-body tw-px-4">
    <div class="tw-overflow-x-auto tw-mb-6">
      ...
      <div class="tw-grid tw-grid-cols-2 tw-gap-4 tw-px-4 tw-text-sm">
        <div>
          <p class="tw-font-semibold">Name: (<span class="tw-text-gray-700 tw-font-normal">{{ \App\Models\User::find($vendor->assigned_to)->firstName }} {{ \App\Models\User::find($vendor->assigned_to)->lastName }}</span>)</p>
        </div>
        <div>
          <p class="tw-font-semibold">Role: (<span class="tw-text-gray-700 tw-font-normal">{{ \App\Models\User::find($vendor->assigned_to)->roles->pluck('name')->first() }}</span>)</p>
        </div>
        <div>
          <p class="tw-font-semibold">Email: (<span class="tw-text-gray-700 tw-font-normal">{{ \App\Models\User::find($vendor->assigned_to)->email }}</span>)</p>
        </div>
        <div>
          <p class="tw-font-semibold">Approved Time: (<span class="tw-text-gray-700 tw-font-normal">{{ $vendor->updated_at->setTimezone('Asia/Manila')->format('h:i A') }}</span>)</p>
        </div>
        <div>
          <p class="tw-font-semibold">Approved Date: (<span class="tw-text-gray-700 tw-font-normal">{{ $vendor->updated_at->setTimezone('Asia/Manila')->format('M d, Y') }}</span>)</p>
        </div>
      </div>
